...
string RSA::encryptChunk(const string &chunk, const Key &key)
{
	...
	//std::cout << "encoded chunk: " << a << "\n";
	// The RSA encryption algorithm is a congruence equation. 
	//std::cout << "GetExponent " << key.GetExponent() << "GetModulus " << key.GetModulus();
					...
					const Key &key)
{
	...
	const unsigned long int fileSize = (const unsigned long int)source.tellg();
